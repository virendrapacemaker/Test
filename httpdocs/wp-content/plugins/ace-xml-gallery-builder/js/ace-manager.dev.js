...
function ace_page_change() {
  var theID = jQuery('#gallery_id').val();
  if (theID != '-1') {
    jQuery('#gallery_prev').val(pageURLs[theID]);
    jQuery('#gallery_prev_p').html(pageURLs[theID]);
    if (pageCodes[theID]) {
      jQuery('#ace_insertcode').hide();
    } else {
      jQuery('#ace_insertcode').show();
      jQuery('#a_insert_shortcode').attr('href', 'admin.php?page=ace-xml-gallery-builder&insert_shortcode=' + theID);
    }
  } else {
    if (window.confirm(acemgr.newpage)) window.location = "../wp-admin/post-new.php?post_type=page";
  }
}

function ace_random_change() {
...
function ace_refreshFolder() {
	folderForm = jQuery('#sort_images_form' );
	data = {
    action: 'ace_refresh_folder',
    _wpnonce: jQuery('#_wpnonce' ).val(),
    folder : jQuery( "input[name='directory']", folderForm ).val(),
    ace_paged : jQuery( "input[name='ace_paged']", folderForm ).val()
  }
...
function ace_admin_list_folder() {
  if (jQuery('.ace_admin_list_folders').length) {
    jQuery('.ace_admin_list_folders').each(function () {
      var thisFolder = jQuery(this).attr('title');
      var data = {
        action: 'ace_admin_list_folders',
        folder: thisFolder
      };
...
function nextDatabase( toGo, toDo ) {
  ...
  jQuery.post( ajaxurl, data, function( response ) { 
    ...
    if ( 0 < toGo ) {
      ...
      jQuery('#rebuild-database').html( acemgr.rebuildReady );
    }        
  });
}
...
function nextRebuild( toGo, toDo, toCache ) {
  ...
  jQuery.post( ajaxurl, data, function( response ) {
    ...
    if ( 0 < toGo ) {
      ...
      jQuery('#rebuild-cache').html( acemgr.rebuildReady );
    }        
  });
}
...
jQuery(document).ready(function () {  
  ...
  if (jQuery('#ace_context').length) {
    var data = {
      action: 'ace_admin_contextmenu',
      folder: jQuery('#ace_context').attr('title')
    };
    jQuery.post(ajaxurl, data, function (response) {
      if ('none' != response) {
        jQuery('#ace_context').html(response);
        jQuery('#ace_context').attr('title', '');
  ...
  jQuery(".to_top,.to_bottom").click(function () {
    ...
    ace_onDrop(table, row);
  });

  /* ajax insert new folder for upload tabs */
  jQuery('.ace_upload-insertfolder').live( 'click', function () {
    var theID = jQuery(this).attr('id');
    var folderID = theID.substr(6);
    var folderDir = jQuery(this).attr('rel');
    var ajaxImg = '#ajax-loading_' + folderID;
    var actionsDiv = '#ace_actions_' + folderID;
    jQuery(ajaxImg).show();
    var data = {
      action: 'ace_insert_folder_shortcode',
      folder: folderDir
    };
  ...
  jQuery('.ace_upload-showfolder').live( 'click', function () {
    var theID = jQuery(this).attr('id');
    var folderID = theID.substr(6);
    var folderDir = jQuery(this).attr('rel');
    var ajaxImg = '#ajax-loading_' + folderID;
    var actionsDiv = '#ace_actions_' + folderID;
    var backDiv = '#ace_back_' + folderID;
    var current_URL = String(window.location) + '&folder=' + folderID;
    jQuery(ajaxImg).show();
    var data = {
      action: 'ace_upload_showfolder',
      folder: folderDir,
	...
	jQuery('.ace_upload-insertimage').live( 'click', function () {
	  var theID = jQuery(this).attr('id');  
	  var imageID = theID.substr(6);
	  var ajaxImg = '#ajax-loading_' + imageID;
	  var actionsDiv = '#ace_actions_' + imageID;
	  var itemID = '#media-item-' + imageID;
	  var pinkyID = '#pinky_' + imageID;
	  var backID = '#ace_back_' + imageID;
	  jQuery(ajaxImg).show();
	  var data = {
	    action: 'ace_insert_image_shortcode',
	    image: jQuery(this).attr('rel')
	  };
	  jQuery('.ace-item').each(function () {
  ...
  if ( isMANUAL != 'MANUAL' ) {
	  ...
	  jQuery('.acen_button').click(function () {
	    if ('' == jQuery('#ace_nw_folder').val()) return false;
	    jQuery('#acen_ajax-loading').show();
	    var data = {
	      action: 'ace_folder_newfolder',
	      _wpnonce: jQuery('#_wpnonce').val(),
	      folder: jQuery('#folder_id').val(),
	      directory: jQuery('#directory').val(),
	      new_folder_name: jQuery('#ace_nw_folder').val(),
	      create_new_folder: 'Add'
	    }
	    var success = acemgr.foldersuccess.replace('%s', jQuery('#ace_nw_folder').val());
	    ...
	    jQuery.post(ajaxurl, data, function (response) {
	      if ( 0 < response.indexOf('postbox')) {
		    ...
		    jQuery('#acen_ajax-loading').hide( function() {      	  
					ace_admin_list_folder();
					ace_folder_subcount();
	      });
  ...
  /* ajax delete cache */
  jQuery('#ace_clear-thumbs').click(function () {
    jQuery('#acea_ajax-loading').show();
    var data = {
      action: 'ace_clear_thumbs',
      folder: jQuery('#folder_id').val(),
      directory: jQuery('#directory').val(),
      file_to_delete: acemgr.thumbs,
      cache: 'thumbs'
    }
    jQuery.post(ajaxurl, data, function (response) {
  ...
  /* view switcher */
  jQuery('#ace-view-switch a').click(function () {
  ...
  jQuery('#set_viewer_level').click(function () {
    ...
    jQuery.post(ajaxurl, data, function (response) {
      if ('true' != response) {
        ...
        jQuery('#ajax-div').html('<div id="message" class="updated"><p>' + acemgr.viewersuccess + '</p></div>');
      }
      jQuery('#message').remove();
    ...
    if ( jQuery('#not-managers option:selected').length ) {  
      ...
      jQuery('#not-managers option:selected').each(function () {
        data = {
          action: 'ace_add_user',
          _wpnonce: jQuery('#_wpnonce').val(),
          ace_settings: jQuery('#ace_settings').val(),
          id: jQuery(this).val(),
          type: 'manager'
        }
        var option = jQuery(this);
  ...
  jQuery('#add-fauthor').click( function() {
     if ( jQuery('#not-authors option:selected').length ) {   
      ...
      jQuery('#not-authors option:selected').each(function() {
        data = {
          action: 'ace_add_fauthor',
          _wpnonce: jQuery('#_wpnonce').val(),
          folder: jQuery('#manage_folder').val(),
          id: jQuery(this).val()
        }
        var option = jQuery(this);
        jQuery.post(ajaxurl, data, function (response) {
  ...
  if ( jQuery('#cache-bar').length ) {
	  jQuery('#cache-bar').progressBar( { boxImage: acemgr.boxImage, barImage: acemgr.barImage } );
  ...
  if ( jQuery('#database-bar').length ) {
	  jQuery('#database-bar').progressBar( { boxImage: acemgr.boxImage, barImage: acemgr.barImage } );
	  jQuery('#rebuild-database').click( function() {      
	    jQuery('#database-bar').css('visibility', 'visible' );
	    var data = {
	        action: 'ace_truncate_table',
	        _wpnonce: jQuery('#ace_settings').val()        
	    }     
	    jQuery.post( ajaxurl, data, function( response ) {
	      if ( '1' != response ) {
	        jQuery('#ajax-div').html( '<div id="message" class="error"><p>'+response+'</p></div>' );
